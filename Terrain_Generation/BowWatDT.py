...
##Triangulate the Delauney Triangulation
    def triangulate(self):
        ...
        for v in self.vertices:
            ...
            # NIL all repeated edges in list
            # these edges are shared bad triangle edges
            # Valid Edges will be the boundary of the star polygon afterwards
            for j in range(0, len(self.edges) - 1):
                for k in range(j + 1, len(self.edges)):
                    if(self.edges[j].start == self.edges[k].end and self.edges[j].end == self.edges[k].start):
                        self.edges[j].start = [None, None]
                        self.edges[j].end = [None, None]
                        self.edges[k].start = [None, None]
                        self.edges[k].end = [None, None]

                    if(self.edges[j].start == self.edges[k].start and self.edges[j].end == self.edges[k].end):
                        self.edges[j].start = [None, None]
                        self.edges[j].end = [None, None]
                        self.edges[k].start = [None, None]
                        self.edges[k].end = [None, None]
        ...
        for t in self.triangles:
            if ((t.ptA.x == self.s0.x and t.ptA.y == self.s0.y) or (t.ptA.x == self.s1.x and t.ptA.y == self.s1.y) or (t.ptA.x == self.s2.x and t.ptA.y == self.s2.y)):
                continue
            elif ((t.ptB.x == self.s0.x and t.ptB.y == self.s0.y) or (t.ptB.x == self.s1.x and t.ptB.y == self.s1.y) or (t.ptB.x == self.s2.x and t.ptB.y == self.s2.y)):
                continue
            elif ((t.ptC.x == self.s0.x and t.ptC.y == self.s0.y) or (t.ptC.x == self.s1.x and t.ptC.y == self.s1.y) or (t.ptC.x == self.s2.x and t.ptC.y == self.s2.y)):
                continue
            else:
